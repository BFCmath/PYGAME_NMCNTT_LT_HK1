...
class Game_logic():
    def __init__(self, NumberWidthCell, NumberHeightCell, Board, Player1, Player2):
        ...
        self.Turn_Color = (227,207,87)
        self.not_Turn_Color = "white"
    ...
    def DrawBoard(self, SCREEN):
        pygame.draw.rect(SCREEN, (245,245,220), ((self.x0, self.y0), (self.Cell_Size*self.NumberWidthCell, self.Cell_Size*self.NumberHeightCell)))
        for x in range(self.x0, self.xn + 1, self.Cell_Size):
            pygame.draw.line(SCREEN, (0, 0, 0), (x, self.y0), (x, self.yn), 2)
        for y in range(self.y0, self.yn + 1, self.Cell_Size):
            pygame.draw.line(SCREEN, (0, 0, 0), (self.x0, y), (self.xn, y), 2)
    ...
    def PrintInfor(self, SCREEN):
        if self.Turn == 1:
            Player1 = Define.get_font(30).render(f"{self.PlayerX}: X", True, self.Turn_Color)
            ...
            Player2 = Define.get_font(30).render(f"{self.PlayerO}:O", True, self.Turn_Color)
            Rect2 = pygame.Rect(Define.Screen.SCREEN_WIDTH - 30 - Player2.get_width(), 30, Player1.get_width(), Player1.get_height())
            SCREEN.blit(Player2, Rect2)
